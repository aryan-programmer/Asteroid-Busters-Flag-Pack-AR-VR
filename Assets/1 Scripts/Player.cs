...
using Utilities;
public
class Player: Singleton_<Player>
{
	...
	internal
	void RemoveAttacker( AAttacker attacker )
	{
		prevHitTargets.Remove( attacker.gameObject );

	}
	internal
	void LookAtAttacker( AAttacker attacker )
	{

		if(!prevHitTargets.Contains( attacker.gameObject ))
		{
			GameObject missile = missiles.GetRandomElement();
			var missile1 = Instantiate( missile , spawnPoint1.position , transform.rotation , null ).GetComponent<Missile>();
			var missile2 = Instantiate( missile , spawnPoint2.position , transform.rotation , null ).GetComponent<Missile>();
